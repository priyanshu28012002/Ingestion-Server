...
public:
    ...
    void start(int port = 8554)
    {
        server_ = gst_rtsp_server_new(); //Allocates a new RTSP server object.
        gst_rtsp_server_set_service(server_, std::to_string(port).c_str()); //Sets the TCP port (RTSP control channel). Default RTSP port is 554, but 8554 is commonly used for non-root apps.
        gst_rtsp_server_attach(server_, nullptr); // Attaches the server to the GLib main loop.
        // This line:
        // Opens the socket
        // Starts accepting RTSP connections
        // Without this, the server does nothing.
    ...
    GstRTSPMountPoints *mounts()
    {
        ...
        // /cam0 → pipeline factory
        // /cam1 → pipeline factory
        // This is how RTSP URLs are created.
        return gst_rtsp_server_get_mount_points(server_);
    }
